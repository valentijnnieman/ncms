...
app.prepare().then(() => {
  ...
  MongoClient.connect(dbUrl, function(err, client) {
    ...
    // create admin user
    db.collection("users").insertOne({ username: "admin", password: "root" });
  });
  ...
  router.put("/users/:id", async ctx => {
    const userId = ObjectId(ctx.params.id);
  ...
  router.delete("/users/:id", async ctx => {
    ...
    db
      .collection("users")
      .remove({ _id: userId, password: ctx.request.body.password });
  });
